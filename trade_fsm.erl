...
%% ask_negotiate: FSM <-> FSM						
idle({ask_negotiate, OtherPid}, S = #state{}) ->
	Ref = monitor(process, OtherPid),
	notice(S, "~p asked fro a trade negotiation", [OtherPid]),
	{next_state, idle_wait, S#state{other = OtherPid, monitor = Ref}};
idle(Event, Data) ->
	unexpected(Event, idle),
	{next_state, idle, Data}.

%% negotiate: Client <-> FSM	
...
%% make_offer/retract_offer: Client <-> FSM
...
%% make_offer/retract_offer: FSM <-> FSM
...
handle_sync_event(cancel, _From, _StateName, S = #state{}) ->
	...
	{stop, cancelled, ok, S};
%% Note: DO NOT reply to unexpected synchronous calls. Let the call-maker crash!
handle_sync_event(Event, _From, StateName, Data) ->
	unexpected(Event, StateName),
	{next_state, StateName, Data}.
	
%% Respond to the other FSM going down
...
unexpected(Msg, State) ->
	io:format("~p received unknown even ~p while in state ~p~n",
						[self(), Msg, State]).
